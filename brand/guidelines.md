...
- Modo educativo/curioso cuando aplica: dato o cifra sorprendente primero, explicación después. **Varía el gancho de apertura entre posts — no uses siempre "¿Sabías que...?"** (usarlo en casi todos los posts es justo lo que hace que el contenido se sienta plantillado). Alterna con: una afirmación directa sin preámbulo ("Los MRL-G no necesitan cuarto de máquinas."), un escenario/pregunta hipotética ("Imagina liberar toda la azotea de tu edificio..."), una comparación breve, o —cuando el post es de tipo `pregunta_comunidad`— la pregunta misma como primera línea, sin dato previo.
...
- Elementos obligatorios: el logo real se compone automáticamente sobre las imágenes generadas — versión azul/dorado original en layouts de fondo claro (`infografia`), versión blanca en layouts de fondo oscuro (`premium`/`hero`, igual que en las piezas reales), sin caja ni fondo detrás en ningún caso — los agentes no deben describirlo en texto.
